...
async function resolvePreview(track){
  ...
  const url=`https://itunes.apple.com/search?term=${encodeURIComponent(track.search)}&entity=song&limit=5&country=ES`;
  const response=await fetch(url);
  if(!response.ok)throw new Error('No se pudo consultar el catálogo musical');
  const data=await response.json();
  const norm=v=>(v||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
  const artist=norm(track.artist), title=norm(track.title);
  const match=data.results.find(r=>r.previewUrl&&norm(r.artistName).includes(artist.split(' ')[0])&&norm(r.trackName).includes(title.split(' ')[0]))||data.results.find(r=>r.previewUrl);
